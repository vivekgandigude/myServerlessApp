...
class SPListController {
  ...
  getAllTestListItems(request, resultsCallback) {
    this.getListAccesToken().then((response) => {
      get(getItemUrl + "$select=ID,Title,field_1,field_2,field_3&$top=10", {
        headers: {
        ...
        .then((response1) => {
          // console.log(response1.data.d.results);
          //return res.status(200).json(response1.data.d);
          resultsCallback(response1.data.d.results);
        })
        .catch((error) => {
          console.log(error);
        });
...
module.exports = splistController;
